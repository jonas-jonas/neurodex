...
type ModelContextProps = {
  ...
  updateModelFunctionActivator: (modelFunctionId: number, functionId: number) => Promise<boolean>;
  updateModelFunctionData: (modelFunctionId: number, parameterName: string, newData: string) => Promise<boolean>;
};
...
export const ModelContextProvider: React.FC<ModelContextProviderProps> = ({ children, modelId }) => {
  ...
  const deleteModelFunction = async (modelFunctionId: number) => {
    if (model) {
      try {
        const newModel = await actions.deleteFunction(model.id, modelFunctionId);
        setModel(newModel);
        return true;
      } catch (e) {
        return false;
  ...
  const updateModelFunctionActivator = async (modelFunctionId: number, functionId: number) => {
    if (model) {
      try {
        const newModel = await actions.updateModelFunctionActivator(model.id, modelFunctionId, functionId);
        setModel(newModel);
        return true;
      } catch (e) {
        return false;
  ...
  const updateModelFunctionData = async (modelFunctionId: number, parameterName: string, newData: string) => {
    if (model) {
      try {
        const newModel = await actions.updateModelFunctionData(model.id, modelFunctionId, parameterName, newData);
        setModel(newModel);
        return true;
      } catch (error) {
        return false;
